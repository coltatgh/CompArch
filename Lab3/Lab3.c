...
/* LC-3b State info.                                           */
/***************************************************************/
#define LC_3b_REGS 8/*INITIALIZATION CONSTANTS*/
...
/*                                                             */
/***************************************************************/
#define ISR_TABLE 0x0200
#define SYSTEM_REG 0x3000 /*ALSO SSR*/
#define USR_INIT 0xFE00
#define PSR_INIT 0x8002
void initialize(char *ucode_filename, char *program_filename, int num_prog_files) { 
    ...
    CURRENT_LATCHES.SSR = SYSTEM_REG;
    CURRENT_LATCHES.PSR = PSR_INIT;
...
int exVector;
int checkException(int *micro) {
    
    int exception;
    int opcode = getOpcode();

    if( (opcode == 2) || (opcode == 3) || (opcode == 6) || (opcode == 7) || (opcode == 15) ){
        ...
        if(BUS < SYSTEM_REG) {
            ...
            if( (CURRENT_LATCHES.PSR & 0x80) == 0) {
                // In system mode
                exception = 0x00;
            } else if((BUS < ISR_TABLE) && (opcode == 0x25)) {
                // User in vector table during trap
                exception = 0x00;
            } else {
                exception = 0x02;
            }
        ...
        // Unaligned:
        else if( ((opcode == 6) || (opcode == 7)) && (BUS % 2 == 1)){
            exception = 0x03;
        }
    }
    ...
    if(exception != 0){
        NEXT_LATCHES.EX_VALID = 1;
        NEXT_LATCHES.EX_VECTOR = exception;
        memcpy(NEXT_LATCHES.MICROINSTRUCTION, CONTROL_STORE[34], sizeof(int)*CONTROL_STORE_BITS);
   ...
   if(exVector != 0){
       ...
       if(microIRD)
            ...
            switch(microCond){
                ...
                case 4:
                    if((CURRENT_LATCHES.INTR_VALID == 1) && ( (CURRENT_LATCHES.PSR & 0x80) != 0))
                        nextState = microJ + 32;
                    else
                        nextState = microJ;
                    break;
...
void eval_bus_drivers() {
   ...
   */
   ...
   /* Calculate ALU output */
    ...
    SSR_IN = CURRENT_LATCHES.SSR;
    
    PSR_IN = CURRENT_LATCHES.PSR;

    if((CYCLE_COUNT >= 300)&&(INTR_VALID == 0)){
        INTR_VALID = 1;
        INTR_VECTOR = 0x01;
    }

    int vector;
    if(CURRENT_LATCHES.EX_VALID){
        vector = CURRENT_LATCHES.EX_VECTOR;
        NEXT_LATCHES.EX_VALID = 0;
    }
    else if(CURRENT_LATCHES.INTR_VALID){
        vector = CURRENT_LATCHES.INTR_VECTOR;
        NEXT_LATCHES.INTR_VALID = 0xFF;
    }
    else{
        vector = 0;
    }

    int ADDR_IN = vector + ISR_TABLE;
}
   ...
   */
   ...
   int DRIVER_COUNT = 0;
    ...
    if(GetGATE_SSR(micro)){
        BUS = Low16bits(SSR_IN);
        DRIVER_COUNT++;
    }
    if(GetGATE_PSR(micro)){
        BUS = Low16bits(PSR_IN);
        DRIVER_COUNT++;
    }
    if(GetGATE_ADDR(micro)){
        BUS = Low16bits(ADDR_IN);
        DRIVER_COUNT++;
    }
   ...
   if(GetLD_PC(micro)){
    ...
    if(GetLD_SSR(micro)){
        ...
        if(GetSSRMUX(micro) == 0)
            NEXT_LATCHES.SSR = CURRENT_LATCHES.SSR - 2;
        else{
            NEXT_LATCHES.SSR = CURRENT_LATCHES.SSR + 2;
        }
    }
    ...
    if(GetTOGL_PSR(micro)){
        if( (CURRENT_LATCHES.PSR & 0x80) != 0)
            NEXT_LATCHES.PSR = Low16bits(CURRENT_LATCHES.PSR & 0x7F);
        else
            NEXT_LATCHES.PSR = Low16bits(CURRENT_LATCHES.PSR | 0x80);
    }

    exVector = checkException(micro);
}