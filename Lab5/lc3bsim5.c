...
int GetMEMMUX1(int *x)       { return(x[MEMMUX1]); }
int GetMEMMUX2(int *x)       { return(x[MEMMUX2]); }
int GetLD_PTE(int *x)       { return(x[LD_PTE]); }
int GetGATE_PTE(int *x)       { return(x[GATE_PTE]); }
int GetLD_LOAD(int *x)       { return(x[LD_LOAD]); }
int GetLD_STORE(int *x)       { return(x[LD_STORE]); }
...
int getPFN(int PTE){
    int frame = (PTE & PTE_PFN_MASK) >> 9;
    return frame;
}

int setPFN(int PTE, int PFN){
    PTE = PTE & 0xC1FF;
    int frame = PFN << 9;
    PTE = PTE | PFN;
    return PTE;
...
int getProtection(int PTE){
    return (PTE & 0x0008) >> 3;
}

int getValid(int PTE){
    return (PTE & 0x0004) >> 2;
}

int getModified(int PTE){
    return (PTE & 0x0002) >> 1;
}

int setModified(int PTE, bool toSet){
    if(toSet)
        PTE = PTE | (0x0001 << 1);
    else
        PTE = PTE & (0xFFFE << 1);
...
int getReference(int PTE){
    return (PTE & 0x0001);
}

int updatePTE(int PTE){
    /*Set reference bit */
    PTE = PTE | 1;

    if(CURRENT_LATCHES.STORE){
        /*Set modified... Could be a source of error*/
        PTE = PTE | 2;
    }
...
int checkForExceptions() {
    ...
    bool inVecTable = ( getPFN(NEXT_LATCHES.NEW_PTE) == 0 );    /* Colton: Not sure about this */
    bool isUserMode = (getPSRMode() == 1);
    bool isWordAccess = ( (CURRENT_LATCHES.JSTORE == 25) || (CURRENT_LATCHES.JSTORE == 23) );
    bool isOddAddress = ( CURRENT_LATCHES.VA % 2 == 1);
    ...
    if(GetLD_PTE(micro)) {
        ...
        /*check for unaligned:*/
        if(isWordAccess && isOddAddress)
            vector = 0x03;

        /*check for protection exception:*/
        else if( !(isTrap && inVecTable) && (getProtection(NEXT_LATCHES.NEW_PTE) == 0) && isUserMode )
            vector = 0x04;

        /* check for page fault */
        else if(getValid(NEXT_LATCHES.NEW_PTE) == 0)
            vector = 0x02;
    }
...
bool INT_TRIGGERED = false;         /* Colton: Make sure this isn't an issue somewhere */
   ...
   */
   ...
   if(microIRD)
        ...
        switch(microCond){
            ...
            /* Patchwork fix for bad ucode: Mem access in Translate */
            case 6:
                if(CURRENT_LATCHES.READY) {
                    nextState = CURRENT_LATCHES.SEQREG;
                    if(CURRENT_LATCHES.SEQREG == 51)
                        NEXT_LATCHES.SEQREG = 62;
                    else
                        NEXT_LATCHES.SEQREG = 51;
                }
                else
                    nextState = microJ;
                break;

            /* Get back into OG state tree */
            case 7:
                nextState = CURRENT_LATCHES.JSTORE;
                NEXT_LATCHES.JSTORE = 0;
                break;
        }
    }
   ...
   */
   ...
   if((microCond == 1) && (readyCount < 4)){
        ...
        if(GetR_W(micro) == 0){
            ...
            /* WRITE */
            if(GetGATE_PTE(micro))
                writeWord( CURRENT_LATCHES.MAR, CURRENT_LATCHES.NEW_PTE );
            else if(GetDATA_SIZE(micro) == 0)
                writeByte( CURRENT_LATCHES.MAR, CURRENT_LATCHES.MDR );  /* Byte */
            else if(GetDATA_SIZE(micro) == 1)
   ...
   */       
   int *micro = CURRENT_LATCHES.MICROINSTRUCTION;
    ...
    if(GetLD_MAR(micro)){
        int memCalc;
        if(GetMEMMUX2(micro)) {
            int MDRpart = CURRENT_LATCHES.MDR & PTE_PFN_MASK;
            int VApart = CURRENT_LATCHES.VA & PAGE_OFFSET_MASK;
            memCalc = MDRpart + VApart;
        }
        else {
            memCalc = CURRENT_LATCHES.PTBR + 2 * CURRENT_LATCHES.VA;
        }
     
        if(GetMEMMUX1(micro))
            NEXT_LATCHES.MAR = memCalc;
        else
   ...
   if(GetLD_PTE(micro)) {
        NEXT_LATCHES.NEW_PTE = updatePTE(Low16bits(FROM_MEM));
   }
