...
void MEM_stage() {
  ...
  int memData = DATA_logic();
  int writeEn = WE_logic();
...
int DATA_logic(){
  ...
  bool isOdd = ((PS.MEM_ADDRESS & 0x1) == 1);
  int data_in = PS.MEM_ALU_RESULT;
  int data_out;

  if( (datasize == 0) && !isOdd ){
    data_out = data_in & 0x00FF; 
  } 
  else if ( (datasize == 0) && isOdd ){
    data_out = (data_in & 0x00FF) << 8;
  } 
  else if( (datasize == 1) && isOdd) {
    printf("UNALIGNED ACCESS ERROR");
  }
  else{
    data_out = (data_in & 0x0FFFF);
  }
...
int WE_logic(){
  int datasize = Get_DATA_SIZE(PS.MEM_CS);
  bool isOdd = ((PS.MEM_ADDRESS & 0x1) == 1);
  bool isWrite = (Get_DCACHE_RW(PS.MEM_CS) == 1);
  int WE_out;
...
int DATA2_logic(int data_in){
  ...
  if( (datasize == 0) && !isOdd ){
    ...
    data_out = maskAndShiftDown(data_in, 8, 15);  
    data_out = signExtend(data_out, 8); /*SEXT here*/
  } 
  else if( (datasize == 1) && isOdd) {
    printf("UNALIGNED ACCESS ERROR");
  }
...
int BR_logic(){
  ...
  if(Get_BR_OP(PS.MEM_CS)){
    int mem_cc = PS.MEM_CC;
    int mem_ir_cc = maskAndShiftDown(PS.MEM_IR, 9, 11);
...
int ADDRESS_logic() {
  ...
  if(Get_ADDR1MUX(PS.AGEX_CS) == 0){
    ADDR1_MUX_out = PS.AGEX_NPC;
  }else{
    ADDR1_MUX_out = PS.AGEX_SR1;
  }

  /* ADDR2_MUX */
  int ADDR2_MUX_out;
  int ADDR2_MUX_val = Get_ADDR2MUX(PS.AGEX_CS);
  if(ADDR2_MUX_val == 0){
    ADDR2_MUX_out = 0;
  }
  else if(ADDR2_MUX_val == 1){
    ADDR2_MUX_out = signExtend(PS.AGEX_IR & 0x03F, 6);
  }
  else if(ADDR2_MUX_val == 2){
    ADDR2_MUX_out = signExtend(PS.AGEX_IR & 0x01FF, 9);
  }else{
    ADDR2_MUX_out = signExtend(PS.AGEX_IR & 0x07FF, 11);
  }

  /* ADDER */
  int ADDER_IN1 = ADDR1_MUX_out;
  int ADDER_IN2;
  ...
  int ADDER_out = ADDER_IN1 + ADDER_IN2;

  /* ADDRESS_MUX */
  int ADDRESS_MUX_out;
  if(Get_ADDRESSMUX(PS.AGEX_CS) == 0){
    ADDRESS_MUX_out = (PS.AGEX_IR & 0x0FF) << 1;
  } else {
    ADDRESS_MUX_out = ADDER_out;
  }

  return ADDRESS_MUX_out;
}
...
int ALU_logic() {
  ...
  int ALU_in_B = SR2_MUX_out;
  int ALU_out;
  int ALUK_val = Get_ALUK(PS.AGEX_CS);
  if(ALUK_val == 0){
